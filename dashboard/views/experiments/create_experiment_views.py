...
import hashlib
from dashboard.models import (Conversation, Task, Group, Treatment, InboxNotification, Organization, Message, User, GroupMember, Experiment, RFIDAssignment, WeightMeasurement, Collaborator, CalendarEvent, Drug, Strain, Comment, Friend, Cage, Animal, Sample, Dose, Observation, Comment)
...
from dashboard.forms import CustomUserCreationForm, UpdateProfileForm, ExperimentForm, ImportForm, AssignTaskForm, ExperimentBasicInfoForm, ExperimentMetricsForm
import json
...
import csv
from dashboard.models import Invitation, UserAction
...
def drafts(request, org_id):
    ...
    return render(request, 'misc/drafts.html', {
        'org_id': org_id,
        'drafts': drafts,
    })
...
def add_investigators(request, org_id, experiment_id):
    ...
    return render(request, 'misc/add_investigators.html', {
        'users': users,
        'org_id': org_id,
...
def experiment_summary(request, org_id, experiment_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render(request, 'misc/summary.html', {
                'org_id': org_id,
                'experiment': experiment,
                'experiment_id': experiment.id,
...
def import_export_view(request, org_id):
    ...
    return render(request, 'import_export/import.html', context)
...
def export_data(request, org_id):
    ...
    return render(request, 'import_export/export.html', {
        'experiments': experiments_page,
        'search_query': search_query,
        'sort_by': sort_by,
        'order': order,
...
def collaborators(request, org_id, experiment_id):
    ...
    return render(request, 'experiments/collaborators.html', context)
