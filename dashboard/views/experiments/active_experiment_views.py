...
from dashboard.models import (Conversation, RFID, Group, Treatment, Message, User, GroupMember, Task, Strain, Notification, Organization, InboxNotification, Experiment, RFIDAssignment, WeightMeasurement, Collaborator, CalendarEvent, Comment, Friend, Cage, Animal, Sample, Dose, Observation, Comment)
...
from dashboard.forms import CustomUserCreationForm, UpdateProfileForm, ExperimentForm, WeighInImportForm, AssignTaskForm
import json
...
def experiment_list(request, org_id):
    ...
    return render(request, 'experiments/all-experiments.html', {'experiments': experiments})
...
def assign_task(request, org_id, experiment_id):
    ...
    return render(request, 'experiments/assign_task.html', {
        'experiment': experiment,
...
def experiment_home(request, org_id, experiment_id):
    ...
    return render(request, 'misc/experiment-home.html', context)
...
def import_measurements(request, org_id, experiment_id):
    ...
    return render(request, 'import_export/import_measurements.html', {
        'experiment': experiment,
...
def process_import_measurements(request, org_id, experiment_id):
    ...
    if request.method == 'POST' and request.FILES.get('file'):
        ...
        if uploaded_file.name.endswith('.csv'):
            ...
            return render(request, 'import_export/import_confirmation.html', {
                'experiment': experiment,
...
def import_details(request, org_id, experiment_id):
    ...
    return render(request, 'import_export/import_details.html', {
        'experiment': experiment,
        'org_id': org_id  # Pass org_id to the template
    })
...
def process_import_details(request, experiment_id):
    ...
    if request.method == 'POST' and request.FILES.get('file'):
        ...
        if uploaded_file.name.endswith('.csv'):
            ...
            # Pass the parsed data to the confirmation page
            parsed_data_json = json.dumps(parsed_data)  # Convert to JSON for later use
            return render(request, 'import_export/import_confirmation_details.html', {
                'experiment': experiment,
                'parsed_data': parsed_data,
                'parsed_data_json': parsed_data_json,
                'uploaded_file': uploaded_file.name  # Pass file name for display
...
def experiment_summary(request, experiment_id, org_id):
    ...
    return render(request, 'misc/summary.html', {'experiment_data': experiment_data, 'experiment_id': experiment_id, 'org_id': org_id })
