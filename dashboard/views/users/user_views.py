...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if not locked:
            ...
            messages.error(request, f"Invalid username or password. {remaining} attempt(s) remaining before 1-minute lock.")
        return redirect(f"{request.path}?username={username}")
...
def research_disclosure_step(request, org_id, disclosure_id, step):
    ...
    base_template = 'admin/base_admin_dashboard.html' if is_admin_suite else 'base/base_dashboard.html'
...
def friend_info(request, org_id, friend_id):
    ...
    return render(request, 'users/friend_info.html', {
        'organization': organization,
        'friend': friend,
...
def user_home(request):
    ...
    return render(request, 'users/userhome.html', {'user': user, 'experiments': experiment_data})
@login_required
@require_POST
...
def request_demo(request):
    ...
    return render(request, 'marketing/request_demo.html', {'submission_success': submission_success})
...
def fill_pdf_template(request, org_id, template_id):
    ...
    return render(request, 'import_export/fill_pdf_template.html', {
        'pdf_template': pdf_template,
...
def fill_out_form(request, org_id, form_id):
    ...
    return render(request, 'import_export/fill_form.html', {
