...
def iacuc_fill_out(request, submission_id):
    ...
    for species in submission.species_entries.all():
        if species.surgery:
            ...
            surgery_forms[species.species_name] = SurgeryForm(request.POST or None, instance=form_instance)

    for species_name, form in surgery_forms.items():
        if f"save_surgery_{slugify(species_name)}" in request.POST:
            if form.is_valid():
                instance = form.save(commit=False)
                instance.submission = submission
                instance.species = IACUCProtocolSpecies.objects.get(submission=submission, species_name=species_name)
